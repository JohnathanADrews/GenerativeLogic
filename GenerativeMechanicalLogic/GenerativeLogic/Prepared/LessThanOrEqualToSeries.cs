...
{
    /// <summary>
    /// Determines whether the left series is less than or equal to the right series.
    /// </summary>
    public class LessThanOrEqualToSeries : GenericComparisonCreator
    {
        ...
        public override LogicalExpression Expression
        {
            get
            {
                ...
                // f[i] = ( a[i] NAB b[i] ) OR ( ( a[i] XNOR b[i] ) AND f[i-1] )
                node = new LogicalExpression.ExpressionNode(0, null, BinaryOperator.OR, isRoot: true);
                expression.AddNode(node);
                node = new LogicalExpression.ExpressionNode(1, 0, BinaryOperator.NAB, LogicalExpression.ExpressionParameter.Parameter1);
                expression.AddNode(node);
        ...
        public LessThanOrEqualToSeries(IHomogeneousWrapper<IOperandStateProvider> operandWrapper = null)
            : base(operandWrapper)
        {
        }
    }
